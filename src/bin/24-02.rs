...
fn solve(input: &str) -> usize {
    ...
    let initial = &[Location { x: 1, y: 0 }, Location { x: width - 2, y: height - 1 }, Location { x: 1, y: 0 }];
    let mut round = 0;
    let goal = &[Location { x: width - 2, y: height - 1 }, Location { x: 1, y: 0 }, Location { x: width -2, y: height - 1 }];
    for (start, end) in std::iter::zip(initial.iter(), goal.iter()) {
        let mut states = vec![*start];
        while !states.iter().any(|state| *state == *end ) {
