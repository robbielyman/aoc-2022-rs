        ...
        .map(|blueprint| {
            ...
            for i in 0..32 {
                states = states.into_iter()
                    ...
                    .inspect(|(state, robots, last)|
                        println!("stage {}: state: {:?}\nrobots: {:?}\naction {}", i + 1, state, robots, last)
                    )
                    .collect();
                let max_projected = states.iter()
                    .map(|(state, robots, _)| {
                        state.geode + (32 - i) * robots.geode
                    })
                    .max()
                    .unwrap();
                states = states.into_iter()
                    .filter(|(state, robots, _)| {
                        state.geode + (32 - i) * robots.geode + optimism(32 - i) >= max_projected
                    })
                    .collect();
            }
            states.into_iter()
