...
fn round(elves: &mut Vec<Elf>, round_id: usize) {
...
fn solve(input: &str) -> usize {
    ...
    // println!("{}", display(&elves));
    for id in 0..10 {
        round(&mut elves, id);
        // println!("{}", display(&elves));
    }
    let (x_min, x_max, y_min, y_max) = dimensions(&elves);
    let width: usize = (x_max + 1 - x_min).try_into().unwrap();
    let height: usize = (y_max + 1 - y_min).try_into().unwrap();
    width * height - elves.len()
}

fn dimensions(elves: &Vec<Elf>) -> (i64, i64, i64, i64) {
    elves.iter()
        .fold((0, 0, 0, 0), |(x_min, x_max, y_min, y_max), elf| {
            (x_min.min(elf.x), x_max.max(elf.x), y_min.min(elf.y), y_max.max(elf.y))
        })
}
